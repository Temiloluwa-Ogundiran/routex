...
export async function POST(request: NextRequest) {
  ...
  if (!pendingAuth || pendingAuth.mode !== "login") {
    return NextResponse.json(
      { status: false, message: "Your sign-in code has expired. Start again." },
      { status: 400 },
    );
  }

  const payload = await request.json();
  const backendResponse = await fetch(`${apiBaseUrl}/auth/login/verify-otp`, {
    body: JSON.stringify({
